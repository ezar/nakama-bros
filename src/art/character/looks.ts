...
import type { ArmStyle, Build, Expression, LegStyle, Palette, Skeleton } from './rig'
import {
  BUILD, SEG, band, bodyFolds, bodyPoint, drawBody, drawRibbon, hemShape, lag, ribbon,
} from './rig'
...
export interface Look {
   ...
   */
  attackStyle: 'punch' | 'slash' | 'kick' | 'shoot'
...
const panel = (s: Skeleton, uv: Array<[number, number]>, tension = 0.7): Path2D =>
...
/** A wrap at the waist: sash, haramaki, belt. The one horizontal in the figure. */
function waistWrap(
  ctx: CanvasRenderingContext2D,
  s: Skeleton,
  c: Palette['sash'],
...
function limbFormLite(a: Pt, b: Pt, r: number): Path2D {
  const dx = b[0] - a[0]
  const dy = b[1] - a[1]
...
export const LOOKS: Record<CrewId, Look> = {
  ...
  // ── The four remaining Straw Hats ──────────────────────────────────────────
  // These are placeholders built by re-dressing an existing rig entry: correct
  // palette and proportions, but not yet their own silhouettes. They exist so
  // the roster type is complete; the character art pass replaces each one.
  robin: {
    ...LOOKS_BASE.nami,
    name: 'Robin',
    banner: '#7E5CA8',
    pal: P({
      skin: '#F0C8A4', hair: '#2A1E28', shirt: '#5E3E78', trousers: '#2E2438',
      boots: '#1E1828', accent: '#B48CD8', sash: '#3E2E52', coat: '#4A3462',
    }),
  },
  franky: {
    ...LOOKS_BASE.zoro,
    ...
    banner: '#3AC8E0',
    pal: P({
      skin: '#F0C0A0', hair: '#3AC8E0', shirt: '#F2F0E6', trousers: '#2E5AA0',
      boots: '#1E2A44', accent: '#3AC8E0', sash: '#E8A030', coat: '#F2F0E6',
    }),
  },
  brook: {
    ...LOOKS_BASE.sanji,
    ...
    banner: '#E8E4F2',
    pal: P({
      skin: '#EDEAE0', hair: '#2A2230', shirt: '#2C2438', trousers: '#241C30',
      boots: '#161020', accent: '#E8E4F2', sash: '#8E2A3E', coat: '#3A2E4A',
    }),
  },
  jinbe: {
    ...LOOKS_BASE.usopp,
    ...
    banner: '#2E86C1',
    pal: P({
      skin: '#4A8CC8', hair: '#20304A', shirt: '#E4C86A', trousers: '#2E4A72',
      boots: '#1C2A44', accent: '#2E86C1', sash: '#8E2A3E', coat: '#D8A83E',
    }),
  },
}
